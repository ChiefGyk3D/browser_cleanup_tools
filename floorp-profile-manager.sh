...
# List, backup, create, delete, and reset Floorp profiles
...
show_help() {
    cat <<EOF
${BOLD}Floorp Profile Manager${NC}

Manage Floorp profiles — list, backup, create, delete, reset, and restore.
Supports both native and Flatpak installations.
...
Options:
  ...
  --backup-dir <path>      Custom backup directory (default: ~/floorp-backups)
...
Examples:
  ...
  $(basename "$0") restore ~/floorp-backups/floorp-native-default-20260228.tar.gz
EOF
}

BACKUP_DIR="$HOME/floorp-backups"
NATIVE_ONLY=false
FLATPAK_ONLY=false
...
create_profile() {
    ...
    # Generate a random directory name like Firefox/Floorp does
    local random_str
...
INIEOF
    ...
    info "Start $APP_NAME with: floorp -P \"$profile_name\" to use this profile"
}
...
main() {
    ...
    # Check if Floorp is running for write operations
