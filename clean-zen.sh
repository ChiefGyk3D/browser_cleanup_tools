#!/usr/bin/env bash
# clean-zen.sh - Cache and data cleaner for Zen Browser
...
main() {
    ...
    header "Zen Browser Cleanup"

    if browser_is_running "ZEN"; then
        error "Zen Browser is running. Please close it first."
        exit 1
    fi
    ...
    for install_type in native flatpak; do
        ...
        profile_dir=$(get_profile_path "ZEN" "$install_type")
        cache_dir=$(get_cache_path "ZEN" "$install_type")

        [[ ! -d "$profile_dir" ]] && continue

        info "Cleaning Zen Browser ($install_type)..."
        clean_mozilla_profile "$profile_dir" "$cache_dir" "Zen Browser ($install_type)"

        if $deep; then
            while IFS='|' read -r _ _ prof_path; do
                clean_zen_deep "$prof_path"
            done < <(find_mozilla_profiles "ZEN" "${install_type}-only" 2>/dev/null)
        fi

        cleaned=true
        log_info "Cleaned Zen Browser ($install_type)"
    done

    if ! $cleaned; then
        info "Zen Browser not found on this system"
    fi
}
